...
class UserFeedGroup(db.Model):
    """用户Feed分组模型"""
    __tablename__ = "user_feed_groups"
    
    id = Column(Integer, primary_key=True)
    user_id = Column(String(32), nullable=False, comment="用户ID")
    name = Column(String(100), nullable=False, comment="分组名称")
    sort_order = Column(Integer, default=0, comment="排序顺序")
    feed_count = Column(Integer, default=0, comment="包含的Feed数量")
    
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)
    
    def __repr__(self):
        return f"<UserFeedGroup user_id={self.user_id}, name={self.name}>"